...
def call_llm(user_text):
    ...
    if signals.get("children_age_comfort") is not None and  st.session_state.volunteer_profile["children_age_comfort"] is None:
        st.write("writing children age comfort")
...
def advance_flow(intent, confidence):
    if intent in ["STOP"]:
        st.session_state.state_index = len(STATE_ORDER) - 1  # CLOSE
        st.write("GOING TO STOP")
        return
     
     # Exit intents — do not advance
    if current_state() == "KNOWING_VOLUNTEER" and knowing_volunteer_complete():
        #go to commitment state
        st.write("done with knowing volunteer")
        st.write("Final Extracted Criteria:")
        st.write(st.session_state.volunteer_profile)
        return st.session_st
    else:
